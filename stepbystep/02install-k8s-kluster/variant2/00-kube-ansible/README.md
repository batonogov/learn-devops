...
Копируем ключики в виртуальные машины из [hosts.yaml](hosts.yml):
 ...
 ```shell
ssh-copy-id root@control1.serg.local
ssh-copy-id root@control2.serg.local
ssh-copy-id root@control3.serg.local
ssh-copy-id root@worker1.serg.local
ssh-copy-id root@worker2.serg.local
ssh-copy-id root@worker3.serg.local
```
...
Виртуальное окружение с которым запускался плейбук 
```shell
